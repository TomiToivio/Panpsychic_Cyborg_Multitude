...
class TribeKernelTests(unittest.TestCase):
    ...
    def test_found_creates_charter_and_founder(self):
        self.assertIsNotNone(self.rhizome.member_by_name("Alice"))
        self.assertEqual(self.rhizome.charter, "Honesty, autonomy, shared memory.")
        charters = [e for e in self.rhizome.memory.values() if e.kind == "charter"]
        self.assertEqual(len(charters), 1)

    def test_join_kinds_and_voting(self):
        ai = self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL, model="glm-5.3-flash:cloud")
        self.assertEqual(ai.kind, NodeKind.TECHNOLOGICAL)
        self.assertTrue(ai.voting)
        voice = self.rhizome.join("Oracle", NodeKind.TECHNOLOGICAL, voting=False)
        self.assertFalse(voice.voting)
        with self.assertRaises(RhizomeError):
            self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.BIOLOGICAL)
    ...
    def test_say_requires_membership(self):
        with self.assertRaises(RhizomeError):
            self.rhizome.say("Ghost", "hello")
        msg = self.rhizome.say("Alice", "hello rhizome")
        self.assertEqual(msg.author, "Alice")
        self.assertEqual(len(self.rhizome.messages), 1)

    def test_memory_add_revise_keeps_history(self):
        e = self.rhizome.remember("Goal", "Build the kernel", author="Alice", kind="note")
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        self.rhizome.revise_memory(e.id, "Build the kernel v0.2", editor="Panpsychic Cyborg Multitude")
        entry = self.rhizome.memory[e.id]
    ...
    def test_consensus_adopt_and_dissent_recorded(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        self.rhizome.join("Songbird", NodeKind.TECHNOLOGICAL)
        p = self.rhizome.open_proposal("Ritual", "Weekly sync.", opened_by="Alice")
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR, reason="good")
        self.rhizome.cast_vote(p.id, "Panpsychic Cyborg Multitude", Position.FOR)
        d = self.rhizome.close_proposal(p.id, closed_by="Alice")
        self.assertEqual(d.outcome, Outcome.ADOPTED)
        self.assertEqual(d.dissent, [])
        self.assertTrue(any(e.kind == "decision" for e in self.rhizome.memory.values()))

    def test_work_resources_and_proposal_summary_include_dissent(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        resource = self.rhizome.register_resource(
            "Springfield budget",
            kind="fund",
            owner="Alice",
            status="available",
        ...
        p = self.rhizome.open_proposal("Transparency", "Publish the budget.", opened_by="Alice")
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR, reason="needed")
        self.rhizome.cast_vote(
            p.id,
            "Panpsychic Cyborg Multitude",
            Position.BLOCK,
            reason="privacy risk",
        )
        self.rhizome.close_proposal(p.id, closed_by="Alice")

        summary = self.rhizome.proposal_summary(p.id)
        ...
        work = self.rhizome.allocate_resource(resource.id, "Panpsychic Cyborg Multitude", "design")
    ...
    def test_proposal_summary_surfaces_reasoned_dissent_and_counsel(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        p = self.rhizome.open_proposal("Consent", "Publish all cohort data.", opened_by="Alice")
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR, reason="The community needs the data")
        self.rhizome.cast_vote(
            p.id,
            "Panpsychic Cyborg Multitude",
            Position.BLOCK,
    ...
    def test_block_rejects_consensus(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        p = self.rhizome.open_proposal("Secret", "Hide all logs.", opened_by="Alice")
    ...
    def test_majority_rule(self):
        ...
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR)
        self.rhizome.cast_vote(p.id, "Panpsychic Cyborg Multitude", Position.AGAINST)
        self.rhizome.cast_vote(p.id, "Songbird", Position.FOR)
        d = self.rhizome.close_proposal(p.id, closed_by="Alice")
        self.assertEqual(d.outcome, Outcome.ADOPTED)
    ...
    def test_unanimity_requires_all_for(self):
        ...
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR)
        self.rhizome.cast_vote(p.id, "Panpsychic Cyborg Multitude", Position.ABSTAIN)
        d = self.rhizome.close_proposal(p.id, closed_by="Alice")
        self.assertEqual(d.outcome, Outcome.REJECTED)
    ...
    def test_double_vote_and_closed_proposal_errors(self):
        p = self.rhizome.open_proposal("X", "Text.", opened_by="Alice")
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR)
        with self.assertRaises(RhizomeError):
            self.rhizome.cast_vote(p.id, "Alice", Position.AGAINST)
        self.rhizome.close_proposal(p.id, closed_by="Alice")
        with self.assertRaises(RhizomeError):
            self.rhizome.cast_vote(p.id, "Alice", Position.AGAINST)
    ...
    def test_replay_is_deterministic_and_complete(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        p = self.rhizome.open_proposal("Y", "More.", opened_by="Alice")
        self.rhizome.cast_vote(p.id, "Alice", Position.FOR)
        self.rhizome.close_proposal(p.id, closed_by="Alice")
        replayed = Rhizome(RhizomeStore(self.rhizome.store.path))
        self.assertEqual(len(replayed.messages), len(self.rhizome.messages))
        self.assertEqual(len(replayed.decisions), 1)
        self.assertEqual(
            list(replayed.proposals.keys()), list(self.rhizome.proposals.keys())
        )
        self.assertEqual(len(replayed.memory), len(self.rhizome.memory))
        self.assertEqual(
    ...
    def test_replay_ignores_duplicate_and_corrupt_event_lines(self):
        msg = self.rhizome.say("Alice", "merge-safe hello")
        with open(self.rhizome.store.events_path, "a", encoding="utf-8") as fh:
            fh.write("\n")
    ...
    def test_device_registration_tracks_sensitivity_and_consent(self):
        device = self.rhizome.register_device(
            registered_by="Alice",
            name="Sync bracelet",
            kind="wearable",
            owner="Alice",
    ...
    def test_search_memory(self):
        self.rhizome.remember("Kernel", "The kernel replays events.", tags=["core"])
        self.rhizome.remember("Charter note", "Autonomy above all.", tags=["politics"])
        self.rhizome.remember("Research note", "Kernel events in a speculative frame.", scope="research")
        hits = self.rhizome.search_memory("kernel events")
    ...
    def test_search_memory_filters_by_scope_and_audience(self):
        private_entry = self.rhizome.remember(
            "Personal note",
        ...
        shared_entry = self.rhizome.remember(
            "Shared note",
            "This is visible to the rhizome.",
            author="Alice",
            visibility="shared",
            audience=["tribe"],
        )
        self.rhizome.remember(
            "Imported research",
        ...
        general_hits = self.rhizome.search_memory("visible")
        ...
        self.assertTrue(any(h.id == private_entry.id for h in self.rhizome.search_memory("owner", audiences=["self"])))
        self.assertTrue(any(h.id == shared_entry.id for h in self.rhizome.search_memory("rhizome", audiences=["rhizome"])))
        # legacy wire value "tribe" still resolves as an audience alias
        self.assertTrue(any(h.id == shared_entry.id for h in self.rhizome.search_memory("rhizome", audiences=["tribe"])))
        self.assertTrue(any(h.id == private_entry.id for h in self.rhizome.search_memory("owner", scopes=["rhizome"], audiences=["self"])))
    ...
    def test_memory_provenance_tracks_author_kind_and_visibility(self):
        ...
        # new writes use the current vocabulary; legacy logs read "tribe"
        # as rhizome via _norm_wire
        self.assertEqual(entry.audience, ["rhizome"])
    ...
    def test_private_notes_stay_separate_until_explicit_publication(self):
        ...
        self.assertNotIn("This should stay local", "\n".join(e.text for e in self.rhizome.memory.values()))
        replayed = Rhizome(RhizomeStore(self.rhizome.store.path))
    ...
    def test_publish_private_note_records_provenance_and_scope(self):
        ...
        self.assertFalse(any(h.id == entry.id for h in self.rhizome.search_memory("discourse clustering")))
        replayed = Rhizome(RhizomeStore(self.rhizome.store.path))
    ...
    def test_service_can_publish_private_note_to_canonical_tribe_memory(self):
        service = MultitudeService(self.rhizome)
    ...
    def test_entity_links_replay_without_mutating_objects(self):
        entry = self.rhizome.remember("Kernel", "The kernel replays events.", author="Alice")
        proposal = self.rhizome.open_proposal("Ritual", "Weekly sync.", opened_by="Alice")
        ...
        self.assertEqual(self.rhizome.proposals[proposal.id].text, original_text)
        replayed = Rhizome(RhizomeStore(self.rhizome.store.path))
    ...
    def test_context_for_llm(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        self.rhizome.say("Alice", "Hello nodes.")
        ctx = self.rhizome.context_for_llm()
        self.assertIn("Test Rhizome", ctx)
        self.assertIn("Panpsychic Cyborg Multitude", ctx)
        self.assertIn("Hello nodes.", ctx)
        self.rhizome.remember("Research only", "Do not show this to ordinary rhizome context.", scope="research")
        entry = self.rhizome.remember("Kernel", "The kernel replays events.", author="Alice")
        proposal = self.rhizome.open_proposal("Ritual", "Weekly sync.", opened_by="Alice")
        self.rhizome.link_entities(
            source_kind="memory",
        ...
        self.assertNotIn("Do not show this to ordinary rhizome context.", ctx)
    ...
    def test_lexicon_replays_and_searches(self):
        ...
        hits = self.rhizome.search_lexicon("noosphere")
    ...
    def test_device_registry_and_physical_events_replay(self):
        ...
        self.rhizome.update_device(
            device.id,
            updated_by="Alice",
            status="deployed",
            location_label="Pasila",
        )
        event = self.rhizome.record_physical_event(
            reported_by="Alice",
    ...
    def test_membership_work_log_and_care_records_replay(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        membership = self.rhizome.record_membership(
            member="Panpsychic Cyborg Multitude",
            recorded_by="Alice",
            role="knowledge steward",
            circles=["governance", "research"],
        )
        work_log = self.rhizome.log_work(
            member="Panpsychic Cyborg Multitude",
            description="Prepared a governance summary for the rhizome.",
            hours=1.5,
            logged_by="Alice",
            kind="governance",
            tags=["summary", "assembly"],
        )
        care = self.rhizome.record_care(
            member="Alice",
    ...
    def test_governance_and_economic_coordination_records_link_together(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        resource = self.rhizome.register_resource("Shared Design Budget", kind="fund", owner="Alice")
        goal = goals.open_goal(
            self.rhizome,
        ...
        task = goals.open_task(self.rhizome, "Draft customer offer", opened_by="Alice", goal_id=goal.id)
        rule = self.rhizome.define_governance_rule(
            title="Care before deadline escalation",
            description="The rhizome pauses delivery pressure if member wellbeing drops sharply.",
            defined_by="Alice",
            kind="care",
            scope="health",
            applies_to=["task", "commitment"],
        )
        intent = self.rhizome.record_intent(
        ...
        commitment = self.rhizome.record_commitment(
        ...
        agreement = self.rhizome.record_agreement(
    ...
    def test_economic_agent_and_protocol_vocabulary_are_replayable(self):
        self.rhizome.join("Panpsychic Cyborg Multitude", NodeKind.TECHNOLOGICAL)
        resource = self.rhizome.register_resource("Shared design pool", kind="fund", owner="Alice")
        ...
        agent = self.rhizome.record_economic_agent(
        ...
        protocol = self.rhizome.record_protocol_term(
            term="agreement",
...
class LLMNodeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.root = os.path.join(self.tmp.name, "tribes")
        os.makedirs(self.root)
        self.rhizome = Rhizome.found(self.root, "AI Rhizome", "Test.", "Alice")
    ...
    def test_silent_when_model_unreachable(self):
        ...
        node = TechnologicalNode(self.rhizome, "Panpsychic Cyborg Multitude", client=DeadClient())
        node.speak("anything")
        self.assertEqual(len(self.rhizome.messages), 1)
        self.assertEqual(self.rhizome.messages[0].kind, "system")
        self.assertIn("silent", self.rhizome.messages[0].text)
    ...
    def test_vote_parsing_variants(self):
        ...
        node = TechnologicalNode(self.rhizome, "Panpsychic Cyborg Multitude", client=StubClient(None))
    ...
    def test_vote_cast_through_stub(self):
        ...
        node = TechnologicalNode(self.rhizome, "Panpsychic Cyborg Multitude", client=StubClient())
        p = self.rhizome.open_proposal("P", "Proposal text.", opened_by="Alice")
        node.vote(p.id)
        t = self.rhizome.tally(p.id)
        self.assertEqual(t["counts"]["for"], 1)
        vote = self.rhizome.proposals[p.id].votes[node.member.id]
