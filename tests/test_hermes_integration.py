...
from multitude.tribe import Tribe  # noqa: E402
...
class HermesIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.tribe = Tribe.found(self.root, "Hermes Tribe", "Keep memory honest.", "Alice")
        goals.open_goal(
            self.tribe, "Find customers", "Land one new client.",
            category="business", opened_by="Alice",
        )
        self.tribe.remember("Launch note", "Call the first customer.", author="Alice")
        self.adapter = MultitudeHermesAdapter(self.tribe, agent_name="Panpsychic Cyborg Multitude", model="glm-5.3-flash:cloud")
    ...
    def test_status_and_memory_are_readable(self):
        ...
        self.assertEqual(status["tribe"], "Hermes Tribe")
    ...
    def test_individual_memory_is_separate_from_collective_memory(self):
        ...
        self.assertFalse(any("favorite_hobby" in e.text for e in self.tribe.memory.values()))
        self.assertFalse(any(ev.type == "memory_added" and "favorite_hobby" in json.dumps(ev.payload) for ev in self.tribe.store.replay()))

    def test_draft_does_not_mutate_state(self):
        before_events = len(self.tribe.store.replay())
        draft = self.agent.draft_proposal("test collective memory tomorrow")
        after_events = len(self.tribe.store.replay())
    ...
    def test_default_agent_name_matches_repo_identity(self):
        default_adapter = MultitudeHermesAdapter(self.tribe, model="glm-5.3-flash:cloud")
    ...
    def test_unauthorized_voting_is_rejected(self):
        p = self.tribe.open_proposal("Test", "Proposal.", opened_by="Alice")
    ...
    def test_question_routing_is_grounded_in_state(self):
        self.assertIn("Panpsychic Cyborg Multitude", self.agent.ask("Who are the members of the tribe?"))
        self.assertIn("Find customers", self.agent.ask("What are our current goals?"))
        self.assertIn("Launch note", self.agent.ask("What happened in the tribe during the last week?"))

    def test_unresolved_decisions_preserve_structure(self):
        p = self.tribe.open_proposal("Ritual", "Meet every week.", opened_by="Alice")
    ...
    def test_live_counsel_failure_is_clean(self):
        ...
        self.assertEqual(self.tribe.messages[-1].kind, "system")
        self.assertIn("silent", self.tribe.messages[-1].text)
