# -*- coding: utf-8 -*-
"""Rhizome: the kernel. All state is derived by replaying the event log.
...
# Wire-value compatibility: event logs written before the 2026-09-06
# Tribe->Rhizome rename persist actor/scope/audience values as "tribe".
# New writes use "rhizome"; readers accept both (LEGACY maps old->new).
LEGACY_WIRE_VALUES = {"tribe": "rhizome"}


def _norm_wire(value: str) -> str:
    """Normalize a *vocabulary* wire value (scope/audience/actor-kind).

    Case-insensitive legacy lookup, but returns the value untouched when
    it is not a vocabulary term — author/member names must survive as
    written (they are proper names, not enum values).
    """
    key = (value or "").strip().lower()
    return LEGACY_WIRE_VALUES.get(key, value or "")




class Rhizome:
    """A running rhizome: a RhizomeStore plus replayed state."""

    def __init__(self, store: RhizomeStore) -> None:
        self.store = store
        ...
        self.memberships: dict[str, RhizomeMembershipRecord] = {}
        ...
        self.economy_profiles: dict[str, RhizomeEconomyProfileRecord] = {}
    ...
    def found(
        ...
        store = RhizomeStore.create(root, name, charter)
        rhizome = cls(store)
        rhizome._ensure_member(founder_name, NodeKind.BIOLOGICAL)
        rhizome.remember(
            title="Charter",
    ...
    def _apply(self, type_: str, payload: dict[str, Any]) -> None:
        ...
        if type_ == "member_joined":
            ...
            membership = RhizomeMembershipRecord.model_validate(payload["membership"])
            ...
            record = RhizomeEconomyProfileRecord.model_validate(payload["economy_profile"])
    ...
    def _ensure_member(self, name: str, kind: NodeKind) -> Member:
        ...
        if m is not None:
            if m.kind != kind:
                raise RhizomeError(f"'{name}' already exists as a {m.kind.value} node")
            return m
        m = Member(
            id=new_id("node"),
            name=name,
        ...
        self._emit("member_joined", "rhizome", {"member": m.model_dump()})
        self._emit(
            "membership_recorded",
            "rhizome",
            {
                "membership": RhizomeMembershipRecord(
    ...
    def _require_member(self, name: str) -> Member:
        m = self.member_by_name(name)
        if m is None:
            raise RhizomeError(f"no member named '{name}' - join first")
        return m
    ...
    @property
    def name(self) -> str:
        return self.store.read_config().get("name", "rhizome")
    ...
    def remember(
        ...
        scope_clean = _norm_wire(scope or "rhizome")
        if scope_clean not in {"rhizome", "research", "federated"}:
            raise RhizomeError("memory scope must be rhizome, research, or federated")
        visibility_clean = (visibility or "shared").strip().lower()
        if visibility_clean not in {"shared", "private", "restricted"}:
            raise RhizomeError("memory visibility must be shared, private, or restricted")

        actor_name = _norm_wire(author or "rhizome")
        member = self.member_by_name(actor_name) if actor_name and actor_name != "rhizome" else None
        if member is not None:
            author_kind = "technological" if member.kind == NodeKind.TECHNOLOGICAL else "biological"
        elif human is False:
            author_kind = "technological"
        elif actor_name == "rhizome":
            author_kind = "system"
        else:
        ...
        self._emit("memory_added", author or "rhizome", {"entry": entry.model_dump()})
        return entry

    def revise_memory(self, entry_id: str, new_text: str, editor: str) -> MemoryEntry:
        entry = self.memory.get(entry_id)
        if entry is None:
            raise RhizomeError(f"no memory entry '{entry_id}'")
    ...
    def search_memory(
        ...
        """Naive lexical search - good enough for small rhizomes, zero deps."""
        wanted_scopes = {
            _norm_wire((item or "").strip())
            for item in (scopes or ["rhizome"])
            if (item or "").strip()
        }
        wanted_audiences = {
            _norm_wire((item or "").strip())
        ...
        for e in self.memory.values():
            if wanted_scopes and _norm_wire(e.scope).lower() not in wanted_scopes:
                continue
            if wanted_audiences and not {
                _norm_wire(a).lower() for a in (e.audience or [])
            } & wanted_audiences:
                continue
    ...
    def add_private_note(
        ...
        *,
        ...
        if not note.title:
            raise RhizomeError("private note title cannot be empty")
        if not note.text:
            raise RhizomeError("private note text cannot be empty")
    ...
    def publish_private_note(
        ...
        *,
        ...
        if note is None or note.owner_id != owner_member.id:
            raise RhizomeError(f"no private note '{note_id}' for '{owner_member.name}'")
        ...
        return self.remember(
            ...
            visibility="shared" if _norm_wire(scope) == "rhizome" else "restricted",
    ...
    def _resolve_entity_ref(self, kind: str, value: str) -> EntityRef:
        ...
        if not kind_clean:
            raise RhizomeError("entity kind cannot be empty")
        if not value_clean:
            raise RhizomeError("entity id cannot be empty")
        ...
        if kind_clean == "message":
            ...
            raise RhizomeError(f"no message '{value_clean}'")
        if kind_clean == "assemblage":
            if value_clean not in self.assemblages:
                raise RhizomeError(f"no assemblage '{value_clean}'")
    ...
    def link_entities(
        ...
        *,
        ...
        if not relation_clean:
            raise RhizomeError("relation cannot be empty")
        if source_ref == target_ref:
            raise RhizomeError("cannot link an entity to itself")
        link = EntityLink(
            id=new_id("link"),
            ts=now_iso(),
            source=source_ref,
    ...
    def entity_links_for(
        ...
        *,
        ...
        if direction_clean not in {"inbound", "outbound", "both"}:
            raise RhizomeError("direction must be inbound, outbound, or both")
    ...
    def join(
        ...
        if existing is not None:
            if existing.kind != kind:
                raise RhizomeError(f"'{name}' already exists as a {existing.kind.value} node")
    ...
    def leave(self, name: str) -> Member:
        ...
        self.record_membership(
            ...
            status="former",
            notes="Member left the active rhizome roster.",
        )
        return m
    ...
    def record_membership(
        ...
        *,
        ...
        if target is None:
            raise RhizomeError(f"no member named '{member}' - join first")
        actor = self._require_member(recorded_by) if _norm_wire(recorded_by) != "rhizome" else None
        status_clean = (status or "active").strip().lower()
        if status_clean not in {"active", "former", "guest"}:
            raise RhizomeError("membership status must be active, former, or guest")
        record = RhizomeMembershipRecord(
            id=new_id("membership"),
            ts=now_iso(),
            member_id=target.id,
            ...
            status=status_clean,
            recorded_by=actor.name if actor else "rhizome",
            notes=notes.strip(),
            meta=dict(meta or {}),
        )
    ...
    def define_assemblage(
        ...
        *,
        ...
        if not name_clean:
            raise RhizomeError("assemblage name cannot be empty")
        ...
        for raw in components or []:
            if not isinstance(raw, dict) or not raw.get("kind"):
                raise RhizomeError("each assemblage component needs a 'kind'")
    ...
    def update_assemblage(
        ...
        *,
        ...
        if rec is None:
            raise RhizomeError(f"no assemblage '{assemblage_id}'")
        ...
        if status is not None:
            ...
            if status_clean not in {"active", "dissolved"}:
                raise RhizomeError("assemblage status must be active or dissolved")
        ...
        if add_components:
            ...
            for raw in add_components:
                ...
                if comp.label and comp.label.lower() in existing_labels:
                    raise RhizomeError(f"component label '{comp.label}' already present")
    ...
    def define_term(
        ...
        *,
        ...
        if not entry.term:
            raise RhizomeError("term cannot be empty")
    ...
    def register_device(
        ...
        *,
        ...
        if sensitivity_clean not in {"shared", "limited", "private"}:
            raise RhizomeError("device sensitivity must be shared, limited, or private")
        device = DeviceRecord(
            id=new_id("dev"),
            name=name.strip(),
            kind=kind.strip(),
        ...
        if not device.name:
            raise RhizomeError("device name cannot be empty")
        if not device.kind:
            raise RhizomeError("device kind cannot be empty")
    ...
    def update_device(
        ...
        *,
        ...
        if device is None:
            raise RhizomeError(f"no device '{device_id}'")
    ...
    def _normalise_signal_metadata(
        *,
        ...
        if sensitivity_clean not in {"shared", "limited", "private"}:
            raise RhizomeError("signal sensitivity must be shared, limited, or private")
        ...
        if is_sensitive and not consent_required:
            raise RhizomeError("sensitive health or awareness signals require explicit consent")
    ...
    def record_physical_event(
        ...
        *,
        ...
        if not event.event_type:
            raise RhizomeError("physical event type cannot be empty")
        if not event.description:
            raise RhizomeError("physical event description cannot be empty")
    ...
    def record_sensing_event(
        ...
        *,
        ...
        if kind_clean not in {"physical", "biological", "psychic", "cybernetic"}:
            raise RhizomeError("sensing event kind must be physical, biological, psychic, or cybernetic")
        sensitivity_clean, consent_ok, merged_meta = self._normalise_signal_metadata(
            member=m.name,
            signal_type=signal_type,
            source=source,
    ...
    def cast_vote(
        ...
        if p.status != ProposalStatus.OPEN:
            raise RhizomeError(f"proposal '{proposal_id}' is already {p.status.value}")
        m = self._require_member(member)
        if not m.voting:
    ...
    def register_resource(
        ...
        *,
        ...
        owner_name = _norm_wire(owner.strip() if owner else "rhizome")
        if owner_name and owner_name != "rhizome":
        ...
        if not resource.name:
            raise RhizomeError("resource name cannot be empty")
    ...
    def allocate_resource(
        ...
        if resource is None:
            raise RhizomeError(f"no resource '{resource_id}'")
    ...
    def log_work(
        ...
        *,
        ...
        if task_id and task_id not in self.tasks:
            raise RhizomeError(f"no task '{task_id}'")
        if goal_id and goal_id not in self.goals:
            raise RhizomeError(f"no goal '{goal_id}'")
        if contribution_id and contribution_id not in self.contributions:
            raise RhizomeError(f"no contribution '{contribution_id}'")
        if hours <= 0:
            raise RhizomeError("work log hours must be greater than 0")
        ...
        if not record.description:
            raise RhizomeError("work log description cannot be empty")
    ...
    def define_governance_rule(
        ...
        *,
        ...
        record = GovernanceRuleRecord(
            ...
            scope=_norm_wire(scope or "rhizome"),
        ...
        if not record.title:
            raise RhizomeError("governance rule title cannot be empty")
        if not record.description:
            raise RhizomeError("governance rule description cannot be empty")
    ...
    def record_intent(
        ...
        *,
        ...
        if not record.title:
            raise RhizomeError("intent title cannot be empty")
    ...
    def record_commitment(
        ...
        *,
        ...
        if not record.title:
            raise RhizomeError("commitment title cannot be empty")
    ...
    def record_agreement(
        ...
        *,
        ...
        if len(resolved_parties) < 2:
            raise RhizomeError("agreement requires at least two parties")
        commitment_list = []
        for commitment_id in commitment_ids or []:
            if commitment_id not in self.commitments:
                raise RhizomeError(f"no commitment '{commitment_id}'")
    ...
    def define_economy_profile(
        ...
        *,
        ...
        if not value_types:
            raise RhizomeError("economy profile requires at least one recognized value type")
        record = RhizomeEconomyProfileRecord(
            id=new_id("economy"),
            ts=now_iso(),
            created_by=actor.name,
        ...
        if not record.mission:
            raise RhizomeError("economy profile mission cannot be empty")
        if not record.distribution_logic:
            raise RhizomeError("economy profile distribution logic cannot be empty")
        if not record.governance_style:
            raise RhizomeError("economy profile governance style cannot be empty")
        self._emit("economy_profile_defined", actor.name, {"economy_profile": record.model_dump()})
        return record

    def current_economy_profile(self) -> Optional[RhizomeEconomyProfileRecord]:
    ...
    def record_federation_agreement(
        ...
        *,
        ...
        actor = self._require_member(created_by)
        partner_name = partner_rhizome.strip()
        if not partner_name:
            raise RhizomeError("federation partner rhizome cannot be empty")
        if partner_name.lower() == self.name.strip().lower():
            raise RhizomeError("federation partner rhizome must differ from the local rhizome")
        ...
        for agreement_id in related_agreement_ids or []:
            if agreement_id not in self.agreements:
                raise RhizomeError(f"no agreement '{agreement_id}'")
        ...
        if not record.title:
            raise RhizomeError("federation agreement title cannot be empty")
        self._emit(
    ...
    def record_economic_agent(
        ...
        *,
        ...
        if not clean_name:
            raise RhizomeError("economic agent name cannot be empty")
    ...
    def record_protocol_term(
        ...
        *,
        ...
        if not clean_term:
            raise RhizomeError("protocol term cannot be empty")
        clean_definition = (definition or "").strip()
        if not clean_definition:
            raise RhizomeError("protocol definition cannot be empty")
    ...
    def record_care(
        ...
        *,
        ...
        if domain_clean not in {"physical", "mental", "social", "economic"}:
            raise RhizomeError("care domain must be physical, mental, social, or economic")
        record = CareRecord(
            id=new_id("care"),
            ts=now_iso(),
            member_id=target.id,
        ...
        if not record.summary:
            raise RhizomeError("care summary cannot be empty")
    ...
    def define_rhythm(
        ...
        *,
        ...
        if not record.name:
            raise RhizomeError("rhythm name cannot be empty")
        if not record.cadence:
            raise RhizomeError("rhythm cadence cannot be empty")
        if not record.purpose:
            raise RhizomeError("rhythm purpose cannot be empty")
    ...
    def context_for_llm(self, limit: int = 20) -> str:
        ...
        lines.append(f"Rhizome: {cfg.get('name', '?')}")
        ...
        if self.commitments:
            ...
            for record in list(sorted(self.commitments.values(), key=lambda item: item.ts))[-5:]:
                lines.append(f"- {record.title}: {record.owed_by} -> {record.owed_to or 'rhizome'} [{record.status}]")
        if self.agreements:
        ...
        if self.federation_agreements:
            ...
            for record in list(sorted(self.federation_agreements.values(), key=lambda item: item.ts))[-5:]:
                ...
                lines.append(
                    f"- {record.title}: partner={record.partner_rhizome} "
